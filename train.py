...
def parse_args(make_dirs=True):
    ...
    parser.add_argument("--model-type", type=str, default="base", help="Model architecture mode")
    ...
    parser.add_argument("--lr-decay", nargs="+", default=[150, 200], type=int, help="Epoch to learning rate decay")
...
def main_work(rank, world_size, args, logger):
    ...
    model = YoloModel(input_size=args.img_size, num_classes=len(args.class_list), anchors=args.anchors, 
                      model_type=args.model_type, pretrained=not args.scratch).cuda(args.rank)
